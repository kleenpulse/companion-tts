...
export function Transport() {
  ...
  const onVolume = (v: number) => {
    ...
    volumeCommit.current = setTimeout(() => void saveSettings({ volume: v }), 400);
  };
  ...
  return (
    <div style={DECK_TOKENS} className="border-t border-hairline bg-bench-850 px-3 py-2">
      <div className="flex items-stretch gap-2.5">
        <div className="flex min-w-0 flex-1 flex-col justify-center">
          <div className="flex items-center gap-1">
            ...
            <button aria-label="Skip current" onClick={() => void sendEngineCmd({ cmd: "skip" })} className={btn}>
              <SkipForward size={15} />
            </button>
            <button aria-label="Stop and clear queue" onClick={() => void sendEngineCmd({ cmd: "stop" })} className={btn}>
              <Square size={13} />
            </button>

            {queued > 0 && (
              <span className="ml-auto shrink-0 rounded-full border border-accent/40 bg-accent/15 px-2 py-0.5 font-display text-[9px] uppercase tracking-[0.15em] text-accent">
                {queued} queued
              </span>
            )}
          </div>
          ...
          {(degraded || queued > 0) && (
            <div className="mt-1 truncate font-display text-[9px] uppercase tracking-[0.15em]">
              {degraded ? (
                ...
                <span className="text-ink-mute">synthesizing · plays in order</span>
              )}
            </div>
          )}
            ...
            <div className="relative h-2.5 min-w-0 flex-1">
              <div
                ...
                style={{
                  ...
                  WebkitMaskImage: "repeating-linear-gradient(90deg, #000 0 5px, transparent 5px 8px)",
                  maskImage: "repeating-linear-gradient(90deg, #000 0 5px, transparent 5px 8px)",
                }}
              />
              <input
                aria-label="Volume"
